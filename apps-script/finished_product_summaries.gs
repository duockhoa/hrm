const FINISHED_PRODUCT_SUMMARIES_SHEET_NAME = "DANH MỤC THÀNH PHẨM";

function xuatThanhPham() {
  const apiUrl =
    `${DATA_EXPORT_ENV.apiBaseUrl}/finished-product-summaries` +
  ...
  ).getSheetByName(FINISHED_PRODUCT_SUMMARIES_SHEET_NAME);

  if (!sheet) {
    throw new Error(
      `Không tìm thấy tab ${FINISHED_PRODUCT_SUMMARIES_SHEET_NAME}.`,
    );
  }
