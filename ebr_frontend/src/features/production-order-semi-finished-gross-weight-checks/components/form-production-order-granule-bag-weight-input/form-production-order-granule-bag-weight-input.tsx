...
const DOSAGE_FORM_STAGE = "Gói cốm";
...
export default function FormProductionOrderGranuleBagWeightInput({
  productionOrderId,
  itemCode,
  ...
  const latestShellWeightAverageGram =
    getLatestShellWeightAverageGram(shellWeightChecks);
  const orderRequirement = calculateSemiFinishedGrossWeightRequirement(
    productionSpecification,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast.success("Đã lưu kiểm tra khối lượng gói cốm.");
      resetForm();
      await mutate(semiFinishedGrossWeightChecksKey);
      onClose?.();
    } catch (error: any) {
      toast.error(
        getErrorMessage(error, "Không thể lưu kiểm tra khối lượng gói cốm."),
      );
      console.error("Error creating semi-finished gross weight check:", error);
    }
  };
