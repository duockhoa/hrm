...
function ProductionOrderLeakTightnessChecksSkeleton() {
  return (
...
export default function InlineLeakTightnessChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderLeakTightnessCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductionOrderLeakTightnessChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Kiểm tra độ kín</h2>
        ...
        <Table className="min-w-[1180px]">
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              {LEAK_TIGHTNESS_CHECK_KEYS.map((_, index) => (
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="max-w-40 border-r whitespace-normal break-words">
                    {formatText(item.dosage_form_stage)}
                  </TableCell>
                  {LEAK_TIGHTNESS_CHECK_KEYS.map((key) => (
                    <TableCell key={key} className="border-r text-center">
                      {formatResult(item[key])}
                    </TableCell>
                  ))}
