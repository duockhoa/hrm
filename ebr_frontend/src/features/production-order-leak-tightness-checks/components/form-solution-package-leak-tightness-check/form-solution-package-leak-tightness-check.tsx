...
const SOLUTION_PACKAGE_LEAK_TIGHTNESS_REQUIREMENT = [
  "Tần suất kiểm tra: 30 phút/lần.",
  "Gập đôi gói bóp mạnh trong 1 phút, không thấy dịch rỉ ra ở mép túi và vị trí in date.",
].join("\n");

export default function FormSolutionPackageLeakTightnessCheck({
  productionOrderId,
  onClose,
}: {
  ...
  const requirementValue = SOLUTION_PACKAGE_LEAK_TIGHTNESS_REQUIREMENT;
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast.success("Đã lưu kiểm tra độ kín gói dịch.");
      resetForm();
      await mutate(leakTightnessChecksKey);
      onClose?.();
    } catch (error: any) {
      toast.error(
        getErrorMessage(error, "Không thể lưu kiểm tra độ kín gói dịch."),
      );
