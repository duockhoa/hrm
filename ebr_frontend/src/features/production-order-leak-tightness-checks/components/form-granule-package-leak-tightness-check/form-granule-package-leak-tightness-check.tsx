...
const buildGranulePackageRequirement = (baseRequirement: string) =>
  [
    "Tần suất kiểm tra: 30 phút/lần.",
    baseRequirement,
    "Gập đôi gói, bóp gói chìm trong nước không thấy có bọt khí rò rỉ ra khỏi gói.",
  ]
    .filter(Boolean)
    .join("\n");

export default function FormGranulePackageLeakTightnessCheck({
  productionOrderId,
  itemCode,
  ...
  const requirementValue =
    buildGranulePackageRequirement(
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast.success("Đã lưu kiểm tra độ kín gói cốm.");
      resetForm();
      await mutate(leakTightnessChecksKey);
      onClose?.();
    } catch (error: any) {
      toast.error(
        getErrorMessage(error, "Không thể lưu kiểm tra độ kín gói cốm."),
      );
