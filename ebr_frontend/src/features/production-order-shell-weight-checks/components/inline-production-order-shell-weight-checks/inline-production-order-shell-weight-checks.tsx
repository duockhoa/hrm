...
function ProductionOrderShellWeightChecksSkeleton() {
  return (
...
export default function InlineProductionOrderShellWeightChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderShellWeightCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductionOrderShellWeightChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Khối lượng vỏ</h2>
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                ...
                {SHELL_WEIGHT_KEYS.map((_, index) => (
                  <TableHead
                    key={index}
                    className="whitespace-nowrap border-r text-right"
                  >
                    Vỏ {index + 1} ({SHELL_WEIGHT_UNIT})
                  </TableHead>
                ))}
              ...
              {data.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {SHELL_WEIGHT_KEYS.map((key) => (
                      <TableCell
                        key={key}
                        className="whitespace-nowrap border-r text-right"
                      >
                        {formatWeight(item[key]).replace(/,00$/, "")}
                      </TableCell>
                    ))}
