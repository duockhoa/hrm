...
export default function FormProductionOrderHygieneCheck({
  productionOrderId,
  onClose,
}: {
  ...
  const hygieneChecksKey = productionOrderId
    ? API_ROUTES.productionOrders.hygieneChecks(productionOrderId)
    : null;
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      await productionOrdersService.createHygieneCheck(productionOrderId, {
        room_or_equipment: values.room_or_equipment.trim(),
        cleaning_type: values.cleaning_type.trim(),
        result: values.result.trim(),
        note: normalizeOptionalText(values.note ?? ""),
      });

      toast.success("Đã lưu kiểm tra vệ sinh.");
      form.reset(defaultValues);
      await mutate(hygieneChecksKey);
      onClose?.();
    } catch (error: any) {
      toast.error(getErrorMessage(error, "Không thể lưu kiểm tra vệ sinh."));
