...
import { formatDateTime, formatDecimal, getUserLabel } from "../../utils";

function PostPreparationSolutionChecksSkeleton() {
  return (
...
export default function InlineProductionOrderPostPreparationSolutionChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderPostPreparationSolutionCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <PostPreparationSolutionChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Dịch sau pha chế</h2>
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="border-r">Độ trong</TableHead>
              <TableHead className="border-r text-right">pH 1</TableHead>
              <TableHead className="border-r text-right">pH 2</TableHead>
              <TableHead className="border-r text-right">pH 3</TableHead>
              <TableHead>Người nhập dữ liệu</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {data.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="max-w-44 border-r whitespace-normal break-words">
                    {item.solution_clarity ?? ""}
                  </TableCell>
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.solution_ph_1)}
                  </TableCell>
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.solution_ph_2)}
                  </TableCell>
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.solution_ph_3)}
                  </TableCell>
                  <TableCell className="max-w-44 whitespace-normal break-words">
                    {getUserLabel(item.checkedBy)}
                  </TableCell>
                </TableRow>
              );
            })}
