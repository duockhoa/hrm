...
import type { ProductionOrderVolumeCheck } from "../../types";
import {
  formatDateTime,
...
function ProductionOrderVolumeChecksSkeleton() {
  return (
...
export default function InlineProductionOrderVolumeChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderVolumeCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductionOrderVolumeChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Kiểm tra thể tích</h2>
