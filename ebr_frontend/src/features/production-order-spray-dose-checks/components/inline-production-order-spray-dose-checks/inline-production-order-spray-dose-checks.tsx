...
function ProductionOrderSprayDoseChecksSkeleton() {
  return (
...
export default function InlineProductionOrderSprayDoseChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderSprayDoseCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductionOrderSprayDoseChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Số lượng liều xịt</h2>
            ...
            {data.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  {SPRAY_DOSE_KEYS.map((key) => (
                    <TableCell key={key} className="border-r text-right">
                      {formatDoseCount(item[key])}
                    </TableCell>
                  ))}
