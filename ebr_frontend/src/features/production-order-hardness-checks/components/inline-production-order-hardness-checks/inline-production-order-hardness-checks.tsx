...
function ProductionOrderHardnessChecksSkeleton() {
  return (
...
export default function InlineProductionOrderHardnessChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderHardnessCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductionOrderHardnessChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Kiểm tra độ cứng</h2>
            ...
            {data.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  {HARDNESS_KEYS.map((key) => (
                    <TableCell key={key} className="border-r text-right">
                      {formatHardnessWithUnit(item[key], item.unit)}
                    </TableCell>
                  ))}
