...
import type { ProductionOrderSprayDoseCheck } from "../../types";
import {
  SPRAY_DOSE_KEYS,
  formatDateTime,
  formatDoseCount,
...
function SprayDoseCheckDetailSkeleton() {
  return (
...
export default function SprayDoseCheckDetail({
  id,
  onClose,
}: {
  id: string | number;
  ...
  const { data, error } = useSWR<ProductionOrderSprayDoseCheck>(
    API_ROUTES.productionOrders.sprayDoseCheckDetail(id),
    () => productionOrdersService.fetchSprayDoseCheckById(id),
  );

  if (error) {
    return (
      <div className="w-full max-w-4xl rounded border bg-white p-4 shadow-md">
        <DetailPanelHeader title="Số lượng liều xịt" onClose={onClose} />
        <div className="mt-4 rounded border border-dashed p-6 text-center text-sm text-gray-500">
          Không tìm thấy bản ghi kiểm tra số lượng liều xịt.
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return <SprayDoseCheckDetailSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl rounded border bg-white p-4 text-center shadow-md">
      <DetailPanelHeader
        title={`Số lượng liều xịt #${data.id}`}
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        {SPRAY_DOSE_KEYS.map((key, index) => (
          <FieldDisplay
            key={key}
            lable={`Lọ ${index + 1}`}
            value={`${formatDoseCount(data[key])} ${data.unit ?? "liều"}`.trim()}
          />
        ))}
        <FieldDisplay
        ...
        <FieldDisplay
          ...
          value={getUserLabel(data.createdBy)}
        />
      </div>
    </div>
  );
