...
import type { ProductionOrderHardnessCheck } from "../../types";
import {
  HARDNESS_KEYS,
  formatDateTime,
...
import EditHardnessCheckForm from "../edit-hardness-check/edit-hardness-check";

const getErrorMessage = (error: any, fallback: string) =>
  error?.response?.data?.message ?? error?.message ?? fallback;

function HardnessCheckDetailSkeleton() {
  return (
...
export default function HardnessCheckDetail({
  id,
  onClose,
}: {
  id: string | number;
  ...
  const { data, error } = useSWR<ProductionOrderHardnessCheck>(
    API_ROUTES.productionOrders.hardnessCheckDetail(id),
    () => productionOrdersService.fetchHardnessCheckById(id),
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await productionOrdersService.deleteHardnessCheck(data.id);
      toast.success("Đã xóa kiểm tra độ cứng.");
      if (listKey) {
      ...
      toast.error(
        getErrorMessage(deleteError, "Không thể xóa kiểm tra độ cứng."),
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  if (error) {
    return (
      <div className="w-full max-w-4xl rounded border bg-white p-4 shadow-md">
        <DetailPanelHeader title="Kiểm tra độ cứng" onClose={onClose} />
        <div className="mt-4 rounded border border-dashed p-6 text-center text-sm text-gray-500">
          Không tìm thấy bản ghi kiểm tra độ cứng.
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return <HardnessCheckDetailSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl rounded border bg-white p-4 text-center shadow-md">
      <DetailPanelHeader
        title={`Kiểm tra độ cứng #${data.id}`}
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        {HARDNESS_KEYS.map((key, index) => (
          <FieldDisplay
            key={key}
            lable={`Viên ${index + 1}`}
            value={formatHardnessWithUnit(data[key], data.unit)}
          />
        ))}
        <FieldDisplay
