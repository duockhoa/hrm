...
import type { ProductionOrderProductSensoryCheck } from "../../types";
import {
  UNIT_RESULT_KEYS,
  formatDateTime,
...
function ProductSensoryChecksSkeleton() {
  return (
...
export default function InlineProductionOrderProductSensoryChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderProductSensoryCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductSensoryChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Kiểm tra cảm quan sản phẩm</h2>
