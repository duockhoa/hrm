...
import EditProductionOrderSemiFinishedGrossWeightCheckForm from "../edit-production-order-semi-finished-gross-weight-check/edit-production-order-semi-finished-gross-weight-check";
import {
...
function SemiFinishedGrossWeightCheckDetailSkeleton() {
  return (
...
export default function SemiFinishedGrossWeightCheckDetail({
  id,
  onClose,
}: {
  id: string | number;
  ...
  const { data, error } =
    useSWR<ProductionOrderSemiFinishedGrossWeightCheck>(
      API_ROUTES.productionOrders.semiFinishedGrossWeightCheckDetail(id),
      () => productionOrdersService.fetchSemiFinishedGrossWeightCheckById(id),
    );
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await productionOrdersService.deleteSemiFinishedGrossWeightCheck(data.id);
      toast.success("Đã xóa kiểm tra khối lượng cả vỏ.");
      if (listKey) {
      ...
      toast.error(
        getErrorMessage(deleteError, "Không thể xóa kiểm tra khối lượng cả vỏ."),
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  if (error) {
    return (
      <div className="w-full max-w-4xl rounded border bg-white p-4 shadow-md">
        <DetailPanelHeader title="Kiểm tra khối lượng cả vỏ" onClose={onClose} />
        <div className="mt-4 rounded border border-dashed p-6 text-center text-sm text-gray-500">
          Không tìm thấy bản ghi kiểm tra khối lượng cả vỏ.
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return <SemiFinishedGrossWeightCheckDetailSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl rounded border bg-white p-4 text-center shadow-md">
      <DetailPanelHeader
        title={`Kiểm tra khối lượng cả vỏ #${data.id}`}
      ...
      <Dialog modal={false} open={isEditing} onOpenChange={setIsEditing}>
        <DialogContent className="md:max-w-[600px]">
          ...
          <EditProductionOrderSemiFinishedGrossWeightCheckForm
            data={data}
        ...
        {SEMI_FINISHED_GROSS_WEIGHT_KEYS.map((key, index) => {
          ...
          return (
            <FieldDisplay
              ...
              value={weight ? `${weight} ${data.unit ?? "g"}` : ""}
            />
          );
        })}
        <FieldDisplay
