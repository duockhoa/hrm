...
function ProductionOrderPostHomogenizationGranuleChecksSkeleton() {
  return (
...
export default function InlineProductionOrderPostHomogenizationGranuleChecks({
  data,
  selectedCheckId,
  onSelectCheck,
}: {
  data: ProductionOrderPostHomogenizationGranuleCheck[] | undefined;
  selectedCheckId?: string | number | null;
  onSelectCheck?: (checkId: string | number) => void;
}) {
  if (!data) {
    return <ProductionOrderPostHomogenizationGranuleChecksSkeleton />;
  }

  return (
    <div className="w-full max-w-4xl min-w-0 overflow-hidden rounded border bg-white p-4 shadow-md">
      <div className="mb-4 flex items-center gap-3">
        <h2 className="text-lg font-semibold">Cốm sau đồng nhất</h2>
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.bulk_density, 6)}
                  </TableCell>
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.tapped_density, 6)}
                  </TableCell>
                  <TableCell className="border-r">
                    {item.density_unit ?? ""}
                  </TableCell>
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.carr_index, 4)}
                  </TableCell>
                  <TableCell className="border-r text-right">
                    {formatDecimal(item.moisture_percent, 2)}
                  </TableCell>
