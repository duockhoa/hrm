"use client";

import { zodResolver } from "@hookform/resolvers/zod";
import { useEffect } from "react";
import { useForm } from "react-hook-form";
import { toast } from "sonner";
import { mutate } from "swr";
import * as z from "zod";
import { Button } from "@/components/ui/button";
import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { API_ROUTES } from "@/lib/api-routes";
import productionOrdersService from "@/services/product-orders.service";
import type {
  ProductionOrderSemiFinishedNetWeightCheck,
  UpdateSemiFinishedNetWeightCheckPayload,
} from "../../types";
import {
  OPTIONAL_SEMI_FINISHED_NET_WEIGHT_KEYS,
  SEMI_FINISHED_NET_WEIGHT_KEYS,
  formatDosageFormStage,
  getVialCheckType,
  normalizeDecimalText,
  toNumber,
} from "../../utils";

const decimalPattern = /^\d{1,7}([.,]\d{1,3})?$/;

const requiredWeightText = (fieldLabel: string) =>
  z
    .string()
    .trim()
    .min(1, `Vui lòng nhập ${fieldLabel}`)
    .refine((value) => decimalPattern.test(value), {
      message: `${fieldLabel} tối đa 3 chữ số thập phân`,
    })
    .refine((value) => toNumber(value) > 0, {
      message: `${fieldLabel} phải lớn hơn 0`,
    });

const optionalWeightText = (fieldLabel: string) =>
  z
    .string()
    .trim()
    .refine((value) => !value || decimalPattern.test(value), {
      message: `${fieldLabel} tối đa 3 chữ số thập phân`,
    })
    .refine((value) => !value || toNumber(value) > 0, {
      message: `${fieldLabel} phải lớn hơn 0`,
    });

const formSchema = z.object({
  requirement: z.string().trim(),
  unit_1_net_weight: requiredWeightText("khối lượng đơn vị 1"),
  unit_2_net_weight: optionalWeightText("khối lượng đơn vị 2"),
  unit_3_net_weight: optionalWeightText("khối lượng đơn vị 3"),
  unit_4_net_weight: optionalWeightText("khối lượng đơn vị 4"),
  unit_5_net_weight: optionalWeightText("khối lượng đơn vị 5"),
  unit_6_net_weight: optionalWeightText("khối lượng đơn vị 6"),
  unit_7_net_weight: optionalWeightText("khối lượng đơn vị 7"),
  unit_8_net_weight: optionalWeightText("khối lượng đơn vị 8"),
  unit_9_net_weight: optionalWeightText("khối lượng đơn vị 9"),
  unit_10_net_weight: optionalWeightText("khối lượng đơn vị 10"),
});

type FormValues = z.infer<typeof formSchema>;

const toFormValues = (
  data: ProductionOrderSemiFinishedNetWeightCheck,
  requirement?: string,
): FormValues => ({
  requirement: requirement ?? data.requirement ?? "",
  ...SEMI_FINISHED_NET_WEIGHT_KEYS.reduce(
    (values, key) => ({
      ...values,
      [key]:
        data[key] === null || data[key] === undefined
          ? ""
          : String(data[key]),
    }),
    {} as Omit<FormValues, "requirement">,
  ),
});

const getErrorMessage = (error: any, fallback: string) =>
  error?.response?.data?.message ?? error?.message ?? fallback;

export default function EditProductionOrderSemiFinishedNetWeightCheckForm({
  data,
  requirement,
  onCancel,
  onSaved,
}: {
  data: ProductionOrderSemiFinishedNetWeightCheck;
  requirement?: string;
  onCancel: () => void;
  onSaved?: (data: ProductionOrderSemiFinishedNetWeightCheck) => void;
}) {
  const requirementValue = requirement ?? data.requirement ?? "";
  const vialCheckType = getVialCheckType(data);
  const unitLabel =
    vialCheckType === "tube" ? "Tuýp" : vialCheckType ? "Lọ" : "Túi";
  const checkTitle =
    vialCheckType === "vial"
      ? "Cập nhật kiểm tra khối lượng lọ"
      : vialCheckType === "solution"
        ? "Cập nhật kiểm tra khối lượng dịch trong lọ"
        : vialCheckType === "tube"
          ? "Cập nhật kiểm tra khối lượng dịch trong tuýp"
        : "Cập nhật kiểm tra khối lượng tịnh";
  const form = useForm<FormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: toFormValues(data, requirementValue),
  });

  useEffect(() => {
    form.reset(toFormValues(data, requirementValue));
  }, [data, form, requirementValue]);

  const onSubmit = async (values: FormValues) => {
    if (data.id === null || data.id === undefined) {
      toast.error("Không tìm thấy bản ghi kiểm tra khối lượng tịnh.");
      return;
    }

    const payload: UpdateSemiFinishedNetWeightCheckPayload = {
      requirement: values.requirement.trim(),
      dosage_form_stage: formatDosageFormStage(data.dosage_form_stage),
      unit: data.unit || "g",
      unit_1_net_weight: normalizeDecimalText(values.unit_1_net_weight),
    };

    OPTIONAL_SEMI_FINISHED_NET_WEIGHT_KEYS.forEach((key) => {
      const value = values[key].trim();
      payload[key] = value ? normalizeDecimalText(value) : null;
    });

    try {
      const savedData =
        await productionOrdersService.updateSemiFinishedNetWeightCheck(
          data.id,
          payload,
        );

      toast.success("Đã cập nhật kiểm tra khối lượng tịnh.");
      await mutate(
        API_ROUTES.productionOrders.semiFinishedNetWeightCheckDetail(data.id),
      );
      if (data.production_order_id) {
        await mutate(
          API_ROUTES.productionOrders.semiFinishedNetWeightChecks(
            data.production_order_id,
          ),
        );
      }
      onSaved?.(savedData);
    } catch (error: any) {
      toast.error(
        getErrorMessage(
          error,
          "Không thể cập nhật kiểm tra khối lượng tịnh.",
        ),
      );
      console.error("Error updating semi-finished net weight check:", error);
    }
  };

  return (
    <div className="h-[100%] min-h-[620px] rounded-md bg-white p-4 shadow-md">
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className="flex min-h-[588px] flex-col justify-between gap-4"
        >
          <div className="space-y-4">
            <p className="text-center text-xl font-semibold uppercase text-gray-900">
              {checkTitle}
            </p>

            <FormField
              control={form.control}
              name="requirement"
              render={({ field }) => (
                <FormItem>
                  <div className="space-y-2">
                    <FormLabel>Yêu cầu</FormLabel>
                    <FormControl>
                      <Textarea
                        readOnly
                        aria-readonly="true"
                        className="bg-gray-100 text-gray-700"
                        {...field}
                      />
                    </FormControl>
                  </div>
                  <FormMessage />
                </FormItem>
              )}
            />

            <div className="flex flex-col gap-4">
              {SEMI_FINISHED_NET_WEIGHT_KEYS.map((key, index) => (
                <FormField
                  key={key}
                  control={form.control}
                  name={key}
                  render={({ field }) => (
                    <FormItem>
                      <div className="space-y-2">
                        <FormLabel>{unitLabel} {index + 1}</FormLabel>
                        <FormControl>
                          <Input
                            inputMode="decimal"
                            disabled={form.formState.isSubmitting}
                            {...field}
                          />
                        </FormControl>
                      </div>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              ))}
            </div>
          </div>

          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="outline"
              disabled={form.formState.isSubmitting}
              onClick={onCancel}
            >
              Hủy
            </Button>
            <Button
              type="submit"
              disabled={form.formState.isSubmitting}
              className="bg-blue-500 text-white hover:bg-blue-600"
            >
              {form.formState.isSubmitting ? "Đang cập nhật..." : "Cập nhật"}
            </Button>
          </div>
        </form>
      </Form>
    </div>
  );
}
