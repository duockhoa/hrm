...
export const PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_ROUTE =
  '/production-orders/post-homogenization-granule-checks/images';

const PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_UPLOAD_DIR = join(
  process.cwd(),
  'uploads',
  'production-order-post-homogenization-granule-checks',
);
export const PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_UPLOAD_DIR =
  join(PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_UPLOAD_DIR, 'images');

const MAX_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_SIZE_IN_BYTES =
  20 * 1024 * 1024;
...
const ensureProductionOrderPostHomogenizationGranuleCheckUploadDirs = () => {
  mkdirSync(
    PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_UPLOAD_DIR,
    {
      recursive: true,
    },
  );
...
const getStoredExtension = (
  file: PostHomogenizationGranuleCheckUploadFileMetadata,
) =>
  IMAGE_EXTENSIONS_BY_MIME_TYPE.get(file.mimetype) ??
  extname(normalizeOriginalFilename(file.originalname)).toLowerCase();

export const getPostHomogenizationGranuleCheckUploadStoredFilename = (
  file: PostHomogenizationGranuleCheckUploadFileMetadata,
) =>
...
export const productionOrderPostHomogenizationGranuleCheckImageUploadOptions = {
  storage: thumbnailDiskStorage({
    ...
    filename: (_req, file, callback) => {
      callback(
        null,
        getPostHomogenizationGranuleCheckUploadStoredFilename(file),
      );
    },
  }),
  ...
  limits: {
    fileSize: MAX_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_SIZE_IN_BYTES,
  },
};

export const getPostHomogenizationGranuleCheckImagePath = (
  ...
  return `${PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_ROUTE}/${file.filename}`;
};
...
export const getPostHomogenizationGranuleCheckImageLookupPaths = (
  filename: string,
) => {
  const safeFilename = getSafeFilename(filename);
  ...
  return safeFilename
    ? [
        `${PRODUCTION_ORDER_POST_HOMOGENIZATION_GRANULE_CHECK_IMAGE_ROUTE}/${safeFilename}`,
      ]
    : [];
};
