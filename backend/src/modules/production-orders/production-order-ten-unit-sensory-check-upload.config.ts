...
export const PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_ROUTE =
  '/production-orders/ten-unit-sensory-checks/images';

export const PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_UPLOAD_DIR = join(
  process.cwd(),
  'uploads',
...
export const MAX_TEN_UNIT_SENSORY_CHECK_IMAGE_COUNT = 10;
...
const ensureUploadDirectory = () => {
  mkdirSync(PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_UPLOAD_DIR, {
    recursive: true,
  });
};
...
export const productionOrderTenUnitSensoryCheckImageUploadOptions = {
  storage: thumbnailDiskStorage({
    destination: (_req, _file, callback) => {
      ensureUploadDirectory();
      callback(null, PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_UPLOAD_DIR);
    },
...
export const getTenUnitSensoryCheckImagePaths = (
  files?: Express.Multer.File[],
) =>
  files?.map(
    (file) =>
      `${PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_ROUTE}/${file.filename}`,
  ) ?? [];
...
const getResolvedFilePath = (filename: string) => {
  ...
  const uploadDir = resolve(
    PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_UPLOAD_DIR,
  );
  const filePath = resolve(uploadDir, safeFilename);
  return filePath.startsWith(`${uploadDir}${sep}`) ? filePath : null;
};

export const getTenUnitSensoryCheckImageLookupPaths = (filename: string) => {
  const safeFilename = getSafeFilename(filename);
  return safeFilename
    ? [`${PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_ROUTE}/${safeFilename}`]
    : [];
};

export const resolveTenUnitSensoryCheckImageFile = async (
  filename: string,
  original = false,
) => {
...
export const removeUploadedTenUnitSensoryCheckImages = async (
  files?: Express.Multer.File[],
) =>
  Promise.all(files?.map((file) => removeImageAndThumbnail(file.path)) ?? []);

export const removeTenUnitSensoryCheckImagesByPath = async (
    ...
    imagePaths?.map(async (imagePath) => {
      if (
        !imagePath?.startsWith(
          `${PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_ROUTE}/`,
        )
      ) {
        return;
      }
      ...
      const filePath = getResolvedFilePath(
        imagePath.slice(
          PRODUCTION_ORDER_TEN_UNIT_SENSORY_CHECK_IMAGE_ROUTE.length + 1,
        ),
      );
      if (filePath) {
